...
interface Tab { id: string; name: string; unread: number; hasNewMessage: boolean; }
  ...
  return (
    <div className="flex items-center gap-0.5 p-1 overflow-x-auto border-b border-border/30 bg-secondary/20">
      {tabs.map((tab) => {
        ...
        return (
          <div key={tab.id} className={`group flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm cursor-pointer transition-all whitespace-nowrap ${isActive ? "bg-primary/15 text-primary border border-primary/20" : tab.hasNewMessage ? "tab-flash text-accent hover:bg-secondary/50" : "text-muted-foreground hover:bg-secondary/50 hover:text-foreground"}`} onClick={() => onSelect(tab.id)}>
